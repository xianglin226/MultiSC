...
if __name__ == "__main__":
    ...
    adt = adt[:, 2] #1:GATA3  2:FOXP3
    label_cell_indx = np.arange(len(y))
    low = np.quantile(adt,0.25)
    high = np.quantile(adt,0.75)
    ml_ind1, ml_ind2, cl_ind1, cl_ind2 = generate_random_pair(adt, label_cell_indx, low, high, num1=3000, num2=3000)
    ...
    # 在数据预处理时将所有数据转换为FloatTensor
    adata1_X = torch.tensor(adata1.X).float().cuda()
    adata1_raw_X = torch.tensor(adata1.raw.X).float().cuda()
    adata2_X = torch.tensor(adata2.X).float().cuda()
    adata2_raw_X = torch.tensor(adata2.raw.X).float().cuda()
    size_factors1 = torch.tensor(adata1.obs.size_factors).float().cuda()
    size_factors2 = torch.tensor(adata2.obs.size_factors).float().cuda()
    ...
    hcae_weights=args.hcae_weight_file)
    ...
    if args.embedding_file != -1:
       final_latent,_,_ = model.encodeBatch(torch.tensor(adata1.X).float().cuda(), torch.tensor(adata2.X).float().cuda())
